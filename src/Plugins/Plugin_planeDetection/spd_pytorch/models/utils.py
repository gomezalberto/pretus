...
def classification_scores(gts, preds, labels):
    ...
    for label in labels:
        class_accuracies.append(metrics.accuracy_score(gts[gts == label], preds[gts == label]))
    class_accuracies = np.array(class_accuracies)

    f1_micro        = metrics.f1_score(gts,        preds, average='micro', labels=labels)
    precision_micro = metrics.precision_score(gts, preds, average='micro', labels=labels)
    recall_micro    = metrics.recall_score(gts,    preds, average='micro', labels=labels)
    f1_macro        = metrics.f1_score(gts,        preds, average='macro', labels=labels)
    precision_macro = metrics.precision_score(gts, preds, average='macro', labels=labels)
    recall_macro    = metrics.recall_score(gts,    preds, average='macro', labels=labels)

    # class wise score
    f1s        = metrics.f1_score(gts,        preds, average=None, labels=labels)
    precisions = metrics.precision_score(gts, preds, average=None, labels=labels)
    recalls    = metrics.recall_score(gts,    preds, average=None, labels=labels)
